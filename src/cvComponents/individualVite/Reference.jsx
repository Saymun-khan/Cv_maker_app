import React, { useState } from 'react'
import { useDispatch } from 'react-redux'
import { referencesInformation } from '../../redux/action/action'
import CvButton from '../CvButton'
import Input from '../input'

const Reference = () => {
    const init = {
        name:'',
        title:'',
        mobile:'',
        email:''
    }
    const [value,setValue] = useState({...init})
    const [errors,setErrors] = useState({...init})
    const [valid,setValid] = useState(false)
    const [msg,setMsg] = useState(false)
    const [addOnes,setAddOnes] = useState([])

    //dispatch
    const dispatch = useDispatch()

    //handle submit
    const handleSubmit = (e) => {
        const {errors,isValid} = validity(value)
        if(isValid){
            setValid(true)
            setErrors('')
            console.log(value)
            addOne()
            dispatch(referencesInformation(addOnes))
        }else{
            setErrors({...errors})
        }
    }

    //handle change
    const handleChange = (e) => {
        setValue((prev) => ({
            ...prev,
            [e.target.name]:e.target.value
        }))
    }
    //check validity
    const validity = (value) => {
        const errors = {}
        const {name,title,mobile,email} = value
        if(!name){
            errors.name = 'Invalid First Name'
        }
        if(!mobile){
            errors.mobile = 'Invalid mobile number'
        }
        if(!title){
            errors.title = 'Invalid title Name'
        }
        if(!email){
            errors.email = 'Invalid email Name'
        }
        return{
            errors,
            isValid:Object.keys(errors).length === 0
        }
    }

    //add another one
    const addOne = (e) => {
        addOnes.push(value)
        console.log('array data',addOnes)
        setMsg(true)
    }
  return (
    <div>
        <h1 className='text-center font-semibold text-2xl mt-4 text-slate-600'>References Information</h1>
        {msg && <h1 className='text-center font-semibold text-xl mt-4 text-red-700'> One Qualification Added Successfully</h1>}
        <Input 
        Placeholder='Enter Your Name'
        Name='name'
        Value={value.name}
        OnChange={handleChange}
        Error={errors.name}
        />
        <Input 
        Placeholder='Enter Your Title Name'
        Name='title'
        Value={value.title}
        OnChange={handleChange}
        Error={errors.title}
        />
        <Input 
        Placeholder='Enter Your Mobile'
        Name='mobile'
        Value={value.mobile}
        OnChange={handleChange}
        Error={errors.mobile}
        />
        <Input 
        Placeholder='Enter Your Email'
        Name='email'
        Value={value.email}
        OnChange={handleChange}
        Error={errors.email}
        />
        <div className='w-96 h-10 m-auto mt-6'>

            <button className='w-48 h-8 rounded-md bg-emerald-400 mt-4' onClick={addOne}>Add one</button>
            <CvButton Text='Submit' OnClick={handleSubmit} bg='bg-blue-400' />

        </div>
    </div>
  )
}

export default Reference